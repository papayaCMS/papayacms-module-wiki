...
* @version $Id: admin_wiki.php 39818 2014-05-13 13:15:13Z weinert $
*/
...
class wiki_admin extends base_wiki {
  ...
  * Constructor
  */
  function __construct($paramName = "wiki") {
  ...
  */
  function _initContentLanguage() {
    if ($this->language == 0) {
      $this->language = $this->papaya()->administrationLanguage->id;
    }
  }
  ...
  */
  function getXML($layout) {
    $layout->parameters()->set('COLUMNWIDTH_LEFT', '300px');
    if (isset($this->params['cmd'])) {
      switch ($this->params['cmd']) {
      case 'del_article':
        $this->getDeleteArticleForm();
        break;
      }
    }
  ...
  function getArticleList() {
    ...
    foreach ($articles as $data) {
      ...
      $result .= sprintf(
        ...
        papaya_strings::escapeHTMLChars($this->papaya()->images[$glyph]),
  ...
  * @return string XHTML
  */
  function getStatusSelector($name) {
    $this->_initContentLanguage();
